...
##df = pd.read_csv('c:\\users\\rledb\\fuse\\fuse_advisor_business\\scoring_algorithm\\production\\scoring_data_10apr2026.txt',sep='\t',header=0) ## Ultimately switch this to db sql query

participant = [1,'Loren Fox','rledbury@fuse-research.com',62]


def runAIsurvey(participant,mail_link,df):
    path = ai.produceScorecard(participant[0],df)
    print(participant,mail_link,path)
    time.sleep(1)
...
def runTMsurvey(participant,mail_link,df):
    ...
    print(participant,mail_link,path)
    mail.sendEmail(participant,mail_link,path)
